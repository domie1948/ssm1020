            ...
            <tr>
                ...
                <td valign="top" background="image/image/background/content_bg.gif">
                    <table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" background="image/image/background/content_bg.gif">
                        <tr><td height="31"><div class="title">出入库</div></td></tr>
                    </table>
                </td>
                            ...
                            <td colspan="4">
                                <table>
                                    <tr>
                                        ...
                                        <td valign="bottom"><h3 style="letter-spacing:1px;">出入库</h3></td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                                        ...
                                        <td colspan="2">
                                            <form action="user" method="">
                                                <table width="100%"  class="cont tr_color">
                                                    ...
                                                    <c:forEach items="${factoryList.datas }" var="factory" varStatus="vs">
	                                                    <tr align="center" class="d">
	                                                        <td><c:out value="${factory['book']['name']}"></c:out></td>
															<td><c:out value="${factory['num']}"></c:out></td>
															<td>
																<c:if test="${factory['typee'] eq 1}">
																	<c:out value="入库" />
																</c:if>
																<c:if test="${factory['typee'] eq 2}">
																	<c:out value="出库" />
																</c:if>
															</td>
															<td><c:out value="${factory['createtime']}"></c:out></td>
															<td><a href="factory/informationDelete?id=${factory['id'] }" onclick="return window.confirm('您确认删除该信息吗?')">删除</a></td>
													 </tr>
                                                    </c:forEach>
                                                     <tr>
	                                                    <td>
		                                                    <c:if test="${pager.pageNow gt 1}">
											                <a href="factory/informationLoad?pageNow=${pager.pageNow-1 }">上一页</a>
											                </c:if>
											                <c:if test="${pager.pageNow lt pager.totalPage}">
											                <a href="factory/informationLoad?pageNow=${pager.pageNow+1 }">下一页</a>
											                </c:if>
	                                                    </td>
                                                    </tr>
                                                </table>
