            ...
            <tr>
                ...
                <td valign="top" background="image/image/background/content_bg.gif">
                    <table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" background="image/image/background/content_bg.gif">
                        <tr><td height="31"><div class="title">书籍管理</div></td></tr>
                    </table>
                </td>
                ...
                <td valign="top" bgcolor="#F7F8F9">
                    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                            ...
                            <td colspan="4">
                                <table>
                                    <tr>
                                        ...
                                        <td valign="bottom"><h3 style="letter-spacing:1px;">书籍管理</h3></td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td width="2%">&nbsp;</td>
                            <td width="96%">
                            <form action="book/informationLoad" method="post">
                            书籍查询:<input type="text" name="searchName" value="${book['searchName'] }">&nbsp;<input type="submit" value="查询">
                            </form> <a href="request/bookAdd">添加</a>&nbsp; <a href="book/exportExcel">导出报表</a>
                                <table width="100%">
                                    <tr>
                                        <td colspan="2">
                                            <form action="user" method="">
                                                <table width="100%"  class="cont tr_color">
                                                    ...
                                                    <c:forEach items="${bookList.datas }" var="book" varStatus="vs">
	                                                    <tr align="center" class="d">
	                                                        <td><c:out value="${book['name']}"></c:out></td>
															<td><c:out value="${book['language']}"></c:out></td>
															<td><c:out value="${book['num']}"></c:out></td>
															<td><c:out value="${book['price']}"></c:out></td>
															<td><c:out value="${book['business']['name']}"></c:out></td>
															<td><c:out value="${book['createtime']}"></c:out></td>
															<td><a href="request/factoryAdd?book_id=${book['id'] }&typee=1">入库</a> <a href="request/factoryAdd?book_id=${book['id'] }&typee=2">出库</a></td>
															<td><a href="book/informationLoadOne?id=${book['id'] }">编辑</a> <a href="book/informationDelete?id=${book['id'] }" onclick="return window.confirm('您确认删除该信息吗?')">删除</a></td>
													 </tr>
                                                    </c:forEach>
                                                     <tr>
	                                                    <td>
		                                                    <c:if test="${pager.pageNow gt 1}">
											                <a href="book/informationLoad?pageNow=${pager.pageNow-1 }&searchName=${book['searchName']}">上一页</a>
											                </c:if>
											                <c:if test="${pager.pageNow lt pager.totalPage}">
											                <a href="book/informationLoad?pageNow=${pager.pageNow+1 }&searchName=${book['searchName']}">下一页</a>
											                </c:if>
	                                                    </td>
                                                    </tr>
                                                </table>
