                                                ...
                                                <table width="100%"  class="cont tr_color">
                                                    ...
                                                    <c:forEach items="${pager.datas }" var="user" varStatus="vs">
	                                                    <tr align="center" class="d">
	                                                        ...
	                                                        <td><a href="user/queryOne?id=${user['id'] }">编辑</a> <a href="user/userDelete?id=${user['id'] }" onclick="window.confirm('您确定删除该用户吗')">删除</a></td>
	                                                    </tr>
                                                    </c:forEach>
                                                     <tr>
	                                                    <td>
		                                                    <c:if test="${pager.pageNow gt 1}">
											                <a href="user/query?pageNow=${pager.pageNow-1 }">上一页</a>
											                </c:if>
											                <c:if test="${pager.pageNow lt pager.totalPage}">
											                <a href="user/query?pageNow=${pager.pageNow+1 }">下一页</a>
											                </c:if>
	                                                    </td>
                                                    </tr>
                                                </table>
